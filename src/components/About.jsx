...
const ServiceCard = ({ index, title, icon, detail }) => {
  return (
    <Tilt className="xs:w-[280px] w-full">
      ...
      >
        <div
          ...
          className="kyoo-tah-gradient rounded-[20px] py-5 px-11 min-h-[280px] flex justify-evenly items-center flex-col"
        >
          <img src={icon} alt={title} className="w-16 h-16 object-contain" />
          <h3 className="text-white text-[18px] font-bold text-center">
            {title}
          </h3>
          <h3 className="text-[14px] text-center" style={{color: 'orange', minHeight: '110px'}}>
            {detail}
          </h3>
        </div>
      </motion.div>
